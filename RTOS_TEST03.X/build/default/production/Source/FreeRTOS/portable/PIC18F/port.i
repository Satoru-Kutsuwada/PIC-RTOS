# 1 "Source/FreeRTOS/portable/PIC18F/port.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "Source/FreeRTOS/portable/PIC18F/port.c" 2
# 59 "Source/FreeRTOS/portable/PIC18F/port.c"
...
# 5 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\stddef.h" 2 3
...
# 59 "Source/FreeRTOS/portable/PIC18F/port.c" 2
...
# 60 "Source/FreeRTOS/portable/PIC18F/port.c" 2
...
# 63 "Source/FreeRTOS/portable/PIC18F/port.c" 2


# 1 "Source\\rtc_main.h" 1
...
# 65 "Source/FreeRTOS/portable/PIC18F/port.c" 2

RTC_DATA RTCdt;
# 94 "Source/FreeRTOS/portable/PIC18F/port.c"
typedef void TCB_t;
extern volatile TCB_t * volatile pxCurrentTCB;
# 105 "Source/FreeRTOS/portable/PIC18F/port.c"
...
# 450 "Source/FreeRTOS/portable/PIC18F/port.c"
 ...
 *pxTopOfStack = 0x33;
 pxTopOfStack++;
# 482 "Source/FreeRTOS/portable/PIC18F/port.c"
 ...
 *pxTopOfStack = ( StackType_t ) ( ulAddress & ( uint32_t ) 0x00ff );
 pxTopOfStack++;
# 510 "Source/FreeRTOS/portable/PIC18F/port.c"
 *pxTopOfStack = 0x44;
 pxTopOfStack++;
...
# 644 "Source/FreeRTOS/portable/PIC18F/port.c"
 return ( ( BaseType_t ) 1 );
}
...
# 727 "Source/FreeRTOS/portable/PIC18F/port.c"
uint16_t timer_cnt;
uint16_t timer_cnt_buf;
# 795 "Source/FreeRTOS/portable/PIC18F/port.c"
void stack_ptr_disp(void)
{
    ...
    for( j=0; j<3; j++ ){
        ...
        for(i=0; i<128; i++){
            do { { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( *dt >> 4 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( *dt & 0x0f)]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (' '); }; } while (0);
            if((i % 16)==15 ){
...
static void prvTickISR( void )
{
    uint32_t dt;
        ...
        do { if( isr_cnt < 10 ){ const char* _s = ("pxCurrentTCB="); while (*_s) { { while (!TXSTA2bits.TRMT); TXREG2 = (*_s); }; _s++; } { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 28 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 24 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 20 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 16 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 12 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 8 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt >> 4 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( dt & 0x0f)]); }; { while (!TXSTA2bits.TRMT); TXREG2 = ('\r'); }; { while (!TXSTA2bits.TRMT); TXREG2 = ('\n'); }; } } while (0);
        ...
        if( xTaskIncrementTick() != ( ( BaseType_t ) 0 ) ){
            do { if( isr_cnt < 10 ){ const char* _s = ("ISR="); while (*_s) { { while (!TXSTA2bits.TRMT); TXREG2 = (*_s); }; _s++; } { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( 3 >> 4 ) & 0x0f ]); }; { while (!TXSTA2bits.TRMT); TXREG2 = (ConvC[( 3 & 0x0f)]); }; { while (!TXSTA2bits.TRMT); TXREG2 = ('\r'); }; { while (!TXSTA2bits.TRMT); TXREG2 = ('\n'); }; } } while (0);
...
void high_isr_main(void)
{
  ...
  __asm("goto _prvTickISR");
 }

    if( INTCONbits.TMR0IF ){
        __asm("goto _timer0_ISR");
 }
...
{
    __asm("goto _high_isr_main");
}
# 981 "Source/FreeRTOS/portable/PIC18F/port.c"
static void prvSetupTimerInterrupt( void )
{
    const uint32_t ulConstCompareValue = ( ( (( unsigned long ) 48000000) / ( ( uint32_t ) 4 ) ) / ( ( TickType_t ) 1000 ) );
    uint32_t ulCompareValue;
# 994 "Source/FreeRTOS/portable/PIC18F/port.c"
 TMR3H = ( uint8_t ) 0x00;
 TMR3L = ( uint8_t ) 0x00;
...
# 1051 "Source/FreeRTOS/portable/PIC18F/port.c"
}
